...
with st.container():
    ...
    if (uploaded_file is not None): 
        ...
        with st.container():        
            ...
            placeholder = c41            
            with placeholder: 
                cropping_text = '<p Result: style="font-family:Bahnschrift; color:Blue; font-size: 16px;"> <b>Adjust the cropping box and double click to crop out</b></p>'
                c41.markdown(cropping_text, unsafe_allow_html=True)
                cropped_img = st_cropper(image, realtime_update=False, box_color='#0000FF',
                                    should_resize_image=True)  
                
                c51.text(" ")
                c51.text(" ")
                c51.text(" ")
                c51.image(cropped_img,caption="Cropped Image")
                run_predict = True
                if (c51.button("Predict", use_container_width=True) or run_predict):
                    col[0].text("___________________________________________________________________________________________________________")
                    st.session_state.uploaded_image = cropped_img 
                    
                    c4.image(cropped_img, width=250) 
                    
                    index = 0
                    model = utilities.load_model(choosen_model+".h5")
                    if (choosen_model.__contains__("Vgg")):
                        image = cv2.resize(np.array(cropped_img), (224,224))
                    elif (choosen_model.__contains__("Incep")):
                        image = cv2.resize(np.array(cropped_img), (299,299))   
                    ...
                    with st.container():     
                        if (c5.button("Explain", key="explain_original_prediction",use_container_width=True) or st.session_state.run_explain==True):
                            ...
                            with placeholder, st.spinner("In progress..."):  
                                ...
                                #    explanation = utilities.explain_instance(input_arr, label, int(num_samples), model)
                                #st.session_state.explanation = explanation          
                                #img,mask = utilities.explain_instance(input_arr, label, int(num_samples), model)
                                #img,mask = utilities.get_explanation(st.session_state.explanation, label)
                                #utilities.plot_comparison(image, img, mask,c5)
